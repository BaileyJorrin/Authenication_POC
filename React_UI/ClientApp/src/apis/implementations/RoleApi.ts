import IRoleApi from "../interfaces/IRoleApi";
import BaseDataApi from "./BaseDataApi";

class RoleApi extends BaseDataApi implements IRoleApi {
  public endpointName = "Roles";
  ...
  public getAllWithAssignedFunctionalAbilities = async (id: string): Promise<any> => {
    const response = await fetch(`${process.env.REACT_APP_API_STRING}/api/${this.endpointName}/GetAllWithAssignedFunctionalAbilities/${id}`, {
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("idToken")}`,
      },
    });
