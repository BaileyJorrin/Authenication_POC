...
import { ControlPointProps } from "./FunctionalAbilitySelectionsDisplay";
...
export const ControlPointSelectionsDisplay = (props: DisplayProps) => {
  ...
  const [functionalAbilityControlPoints, setFunctionalAbilityControlPoints] =
    React.useState<DataResult>({ data: [], total: 10 });
  const [dataState, setDataState] = React.useState<State>({
    sort: [{ field: "friendlyName", dir: "asc" }],
  });
        ...
        <FormGroup>
          <FormControlLabel
            control={
              <Checkbox
                disabled
                name={checkProps.field}
                checked={checkProps.dataItem[checkProps.field!]}
              />
            }
            label={setDisplayName(checkProps.field)}
          />
        </FormGroup>
      </td>
    );
    ...
    >
      <Fade in={props.modalProps.isOpen}>
        <Box sx={style}>
          ...
          >
            {props.modalProps.name} - Control Points
          </h3>
          {/* @ts-ignore */}
          <Grid
            filterable={false}
